  ...
  return (
    <main className="pt-20">
      ...
      {services.map((service: any, index: number) => {
        ...
        const bgColor = isEven ? '#F7F6F3' : '#EEECEA';
        const displayNumber = `0${service.order || index + 1}`;

        return (
          <section key={service._id} className="py-[120px]" style={{ backgroundColor: bgColor }}>
                ...
                <div className={`lg:col-span-6 relative ${!isEven ? 'lg:col-start-5' : ''}`}>
                  <AnimatedSection>
                    ...
                    <div className="relative z-10">
                      ...
                      {/* Summary */}
                      <h2 className="font-display text-[36px] text-[#0F1113] leading-[1.2] mb-6">
                        {service.summary}
                      </h2>

                      {/* Description */}
                      <p className="text-[17px] text-[#0F1113] opacity-80 leading-[1.8] mb-8">
                        {service.description}
                      </p>
                      ...
                      <ul className="space-y-3 mb-10">
                        {service.bullets.map((bullet: string, bulletIndex: number) => (
                          <li key={bulletIndex} className="flex items-start gap-3">
                            ...
                            <span className="text-[15px] text-[#0F1113] opacity-75">
                              {bullet}
                            </span>
                          </li>
                        ))}
                      ...
                      {/* Outcome Block */}
                      <div className="border-l-[3px] border-[#2F5D62] pl-5">
                        <p className="text-[12px] text-[#A89F91] uppercase tracking-widest mb-2">
                          Outcome
                        </p>
                        <p className="font-display italic text-[20px] text-[#0F1113]">
                          {service.outcome}
                        </p>
                      </div>
                    </div>
      ...
      <section className="bg-[#0F1113] py-[100px]">
        <div className="container-custom max-w-3xl text-center">
          <AnimatedSection>
            <h2 className="font-display text-white text-[40px] leading-[1.3] mb-12">
            ...
            <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
              ...
              <Link
                href="/case-studies"
                className="text-white hover:text-[#2F5D62] transition-colors duration-200 flex items-center gap-2"
              >
